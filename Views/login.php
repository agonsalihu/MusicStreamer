    ...
    <div class="Content">
      ...
      <div class="LoginField">
        <div class="InputFields">
            <div class="loginTitle"><h2>login by Username:</h2></div>
            <form action="../PHP/loginData.php" method="POST" onsubmit="return validateLogin()">
              <div class="input">
                <?php
                  $err_cookie = "errMsg";
                  $err_cookie_value = "User does not exist or Password is incorrect!";
                  $succ_cookie = "successMsg";
                  $succ_cookie_value = "*Password successfully changed!";
                  if(isset($_COOKIE[$err_cookie])) {
                    echo "<label for='text' id='userNotFound' class='errorMsg' style='display: flex;'>$err_cookie_value</label >";
                  }                    
                  if(isset($_COOKIE[$succ_cookie])) {
                    echo "<label for='text' class='successMsg' style='display: flex;'>$succ_cookie_value</label >";
                  }                    
                ?>
                <label for="text" id="errorMsg" class="errorMsg" style="display: none;">*Your Username or Password is Incorrect</label>
                <input id="username" name="loginUsername" type="text" placeholder="Username" onchange="validateUsername()">
                <input id="password" name="loginPassword" type="password" placeholder="Password" onchange="validatePassword()">
                <h4 class="forgotPass"><a href="../Views/resetPassword.php">Forgot Your Password?</a></h4>
